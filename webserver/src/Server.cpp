...
/*   Updated: 2026/01/18 18:49:18 by tching           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Server::Server() : _maxSize(0) {
	_port.insert(8080); // default port
	_serverName = "default_server"; // default server name 
	_errorPages.insert(std::make_pair(400, "www/error/400.html")); //temporary place holder - default error page
	_maxSize = 1024 * 10; // max body size is 1MB
	Location newLocation;
	_locations.push_back(newLocation); // default location
}
...
bool conf_isComment(const std::string &line) { //to check for comment in config file
...
void Server::parseServer(const std::string &serverConfig) {
	std::istringstream stream(serverConfig); // create a stream from serverConfig string
	std::string line; // store each line
	std::string key; // store directive key
	std::string value; // store directive value

	std::getline(stream, line); // skip the server first line {
	while (std::getline(stream, line)) { // reads each line
		if (conf_isComment(line))
			continue;
		std::istringstream lineStream(line); //create a stream from the line
		lineStream >> key; // extract directive key
		if (key == "server_name") {
			if (lineStream >> value)
				_serverName = value;
			else
			...
			while (lineStream >> value) {
				int intVal = strtol(value.c_str(), NULL, 10); //convert string into int
				std::set<int>::iterator it = _port.find(intVal); //check if port exists
				if(it == _port.end()) //if not found, insert new port
					_port.insert(intVal);
			}
		}
			...
			Location newLocation(locationConfig);
